...
find /mnt/project/AntibiotiKU/trimmed_reads/ -type f -name "*_1.fq.gz" | xargs -P 64 -I {} bash -c '
    forward_read="{}"
    reverse_read="$(echo "$forward_read" | sed "s/_1\.fq\.gz$/_2.fq.gz/")"
    sample_name="$(basename "$forward_read" _1.fq.gz)"
    kraken2 --threads 2 --gzip-compressed --db /mnt/project/AntibiotiKU/databases/k2_standard_20231009 --paired $forward_read $reverse_read --output /mnt/project/AntibiotiKU/hunt_kraken2_output/$sample_name.kraken_out --report /mnt/project/AntibiotiKU/hunt_kraken2_output/$sample_name.kraken_report.txt
'
